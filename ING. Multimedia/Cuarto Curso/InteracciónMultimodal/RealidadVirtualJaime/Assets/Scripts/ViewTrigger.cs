...
public class ViewTrigger : MonoBehaviour
{
    ...
    public GameObject cabezaUsuario;
    ...
    void Update()
    {
        ...
        if (Focused)
        {
			...
			if ((mFocusedTime > activationTime) || startAction)
            {
                ...
                //Si no es torre hacemos transición
                if ((triggerType == TriggerType.VR_TRIGGER) || (triggerType == TriggerType.AR_TRIGGER))
                {
                ...
                {
                    cabezaUsuario.transform.position = new Vector3(this.gameObject.transform.position.x,
                        this.gameObject.transform.position.y + 3.0f, this.gameObject.transform.position.z + 0.5f) ;
                }
            }
        }
        else
